...
const loadFileBackedRoom = () => import('./FileBackedRoom')
const FileBackedRoom = lazy(loadFileBackedRoom)
const bytePrefetches = new Map<string, Promise<void>>()
...
function prefetchFileAsset(url: string) {
  const resolvedUrl = resolveAssetUrl(url)
  void loadFileBackedRoom()

  let request = bytePrefetches.get(resolvedUrl)
  if (!request) {
    request = fetch(resolvedUrl, { cache: 'force-cache' })
      .then((response) => {
        if (!response.ok) throw new Error(`Asset prefetch failed: ${response.status}`)
        return response.arrayBuffer()
      })
      .then(() => undefined)
      .catch(() => undefined)
    bytePrefetches.set(resolvedUrl, request)
  }
  ...
  return (
    <group position={room.position as [number, number, number]} rotation={[0, room.rotationY, 0]}>
      ...
      <WorldTextPanel
        ...
        background="#39474b"
        lines={[{
          text: `Renderer pending · ${room.asset.kind === 'scan' ? room.asset.format : room.asset.kind}`,
          size: 44,
          color: '#eef6f7',
          weight: 800,
          direction: 'ltr'
...
function RoomRendererInner({
  ...
  if (room.asset.kind === 'gltf') {
    ...
    return <FileBackedRoom room={room} vendor={vendor} url={room.asset.url} scale={room.asset.scale} />
  }
...
export default function RoomRenderer({ room, vendor }: {
  room: RoomDefinition
  vendor?: Vendor
}) {
