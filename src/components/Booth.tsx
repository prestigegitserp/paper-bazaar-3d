...
function InteractiveGroup({
  ...
  return (
    <group position={position} userData={interaction ? { interaction } : undefined} onClick={onClick}>
      {children}
    </group>
  )
}
...
function PaperStack({
  ...
  return (
    <InteractiveGroup position={position} interaction={interaction}>
      ...
      {[0.16, 0.32, 0.48, 0.64].map((y, i) => (
        <mesh key={y} position={[0, y, 0]} castShadow>
          ...
          <meshStandardMaterial color={i === 3 ? color : '#fffdf7'} roughness={0.75} />
        </mesh>
      ))}
      ...
      {[[-0.38, 0.24, -0.35], [0.38, 0.24, -0.35], [-0.38, 0.24, 0.35], [0.38, 0.24, 0.35]].map(
        (point, i) => (
          <mesh key={i} castShadow position={point as [number, number, number]}>
            ...
            <meshStandardMaterial color="#2b2f36" metalness={0.45} roughness={0.35} />
          </mesh>
        )
      )}
    </group>
...
function Desk({ room, vendor }: { room: RoomDefinition; vendor: Vendor }) {
  ...
  return (
    <InteractiveGroup position={[0.75, 0, 0]} interaction={interaction}>
      <RoundedBox args={[2.4, 0.22, 1.25]} position={[0, 1.18, 0]} radius={0.08} smoothness={4} castShadow>
        <meshStandardMaterial color="#d8c7a5" roughness={0.55} />
      </RoundedBox>
      <mesh position={[0, 0.58, 0]} castShadow>
        <boxGeometry args={[2.05, 1.08, 0.86]} />
        <meshStandardMaterial color={room.theme.primary} roughness={0.45} />
      </mesh>
      <mesh position={[0.15, 1.72, 0]} castShadow>
        <boxGeometry args={[0.95, 0.62, 0.06]} />
        <meshStandardMaterial color="#0b1320" metalness={0.2} roughness={0.25} />
      </mesh>
...
function ProductBoard({ room, vendor }: { room: RoomDefinition; vendor: Vendor }) {
  ...
  return (
    <InteractiveGroup position={[-3.36, 2.65, 0]} interaction={interaction}>
      <mesh castShadow>
        <boxGeometry args={[0.16, 3.15, 5.9]} />
        <meshStandardMaterial color="#08101c" metalness={0.25} roughness={0.26} emissive={room.theme.primary} emissiveIntensity={0.4} />
      </mesh>
  ...
  return (
    <group position={room.position as [number, number, number]} rotation={[0, room.rotationY, 0]}>
      <mesh position={[0, 0.06, 0]} receiveShadow>
        ...
        <meshStandardMaterial color={room.theme.floor} roughness={0.72} />
      </mesh>

      <mesh position={[-3.58, 2.6, 0]} receiveShadow castShadow>
        <boxGeometry args={[0.18, 5.2, 9.5]} />
        <meshStandardMaterial color={room.theme.secondary} roughness={0.72} />
      </mesh>
      ...
      <mesh position={[-2.9, 4.95, 0]} castShadow>
        ...
        <meshStandardMaterial color={room.theme.accent} emissive={room.theme.accent} emissiveIntensity={0.55} toneMapped={false} />
      </mesh>
      ...
      {productA && (
        <group position={[-1.2, 0, -2.65]}>
          <mesh position={[0, 0.52, 0]} castShadow>
            ...
            <meshStandardMaterial color={room.theme.primary} roughness={0.5} />
          </mesh>
      ...
      <pointLight position={[1.5, 4.4, 0]} intensity={22} distance={9} color={room.theme.accent} />
    </group>
  )
}
