...
function PointHotspot({ position, interaction }: {
...
function attachNodeInteractions(
  ...
  scene.traverse((object) => {
    ...
    if (!object.userData.interaction) object.raycast = () => undefined
  })
}
...
export default function FileBackedRoom({
  room,
  vendor,
  url,
  scale = 1
  ...
  useEffect(() => {
    ...
    void Promise.all(tasks)
      ...
      .catch(() => {
        // GLB material colors remain a safe fallback.
      })
  ...
  }, [gl, invalidate, quality, room.asset, scene])
      ...
      <group scale={scale}>
        ...
        {vendor && room.hotspots.map((hotspot) => {
          ...
          return interaction ? (
            <PointHotspot
              key={hotspot.id}
              position={hotspot.anchor.position}
              interaction={interaction}
            />
          ) : null
        })}
